...
void PubAndSub::callback(const sensor_msgs::LaserScan::ConstPtr& laser)
{
    ...
    for (i = 0; i < freq; i++)
    {
        ...
        if (laser->ranges[i - 1] - laser->ranges[i] >= 2.0 && laser->ranges[i] < 2.0 && laser->ranges[i] != 0)
        {
            //距离突然减小，并且较小距离小于2米且不为0
            if(laser->ranges[i - 1] - laser->ranges[i+1] >= 2.0 &&
                abs(laser->ranges[i] - laser->ranges[i+1]) < 0.2 &&
                abs(laser->ranges[i] - laser->ranges[i+2]) < 0.2 &&
                abs(laser->ranges[i] - laser->ranges[i+3]) < 0.2 &&
                abs(laser->ranges[i] - laser->ranges[i+4]) < 0.2 &&
                laser->ranges[i+4] < 2.0 && laser->ranges[i+4] != 0)
            {
                theta = (i+2) * laser->angle_increment + laser->angle_min;
                cal_point(theta, laser->ranges[i+2], tmp_x, tmp_y);
                
                if ((tmp_x <= 1.5 && tmp_x >= -2) && tmp_y < 2 && tmp_y>-0.2 && tmp_y != 0)
                {
